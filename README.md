...
| Text extraction, OCR path | ~24s median, load-dependent |
...
A cache hit replaces the entire OCR pass with a local file read.
...
5 runs each, medians rather than means. The warm median reproduces an independent
measurement taken hours earlier under different load, which is the main reason to trust it.

> **On absolute timings.** Cold OCR on this machine varies with system load — the same
> document has measured anywhere from 17s to 254s. The benchmark therefore reports medians
> with their range, records the load average it ran under, and prints a warning above a
> threshold. The **ratio** is the robust quantity: cold and warm are inflated by the same
> contention, so it survives conditions the absolutes do not.
...
initialisation (9.66s in this run, once per process) is not avoided by this.
...
A hit removes one LLM round-trip (the NL→SQL call) from the request: **1.47s → 0.46s**
measured over 5 repeats. The answer-phrasing call is not cached, so it stays on the
critical path and accounts for the residual.
